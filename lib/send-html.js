...
function getCompiledTemplate(data, options) {
  ...
  if (statusCode && Array.isArray(htmlOptions.errors)) {
    var match = htmlOptions.errors.find(function(error) {
      var found = false;
...
module.exports.configure = function configureHtml(options) {
  ...
  views
    .filter(function(value, index) { return views.indexOf(value) == index; })
    .forEach(function(view) {
      debug('template %s path=%s', view, htmlOptions.viewPath);
